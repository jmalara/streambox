...
# Applies system tweaks to Ugoos AM9 Pro via ADB. Optionally deploys Kodi config.
#
# Prerequisites:
...
#
# Usage:
#   ./scripts/setup-adb.sh                    # System tweaks only (default — for TiviMate setups)
#   ./scripts/setup-adb.sh --with-kodi        # System tweaks + Kodi advancedsettings.xml
#   ./scripts/setup-adb.sh --tivimate-only    # Alias for default (explicit)
...
SKIP_SYSTEM=false
SKIP_KODI=true  # Default: skip Kodi (TiviMate-only setup)
DRY_RUN=false

for arg in "$@"; do
  case $arg in
    --skip-system-tweaks) SKIP_SYSTEM=true ;;
    --with-kodi) SKIP_KODI=false ;;
    --skip-kodi-config|--tivimate-only) SKIP_KODI=true ;;
    --dry-run) DRY_RUN=true ;;
    --help|-h)
      echo "Usage: $0 [--with-kodi] [--skip-system-tweaks] [--dry-run]"
      echo ""
      echo "Options:"
      echo "  --with-kodi           Also deploy Kodi advancedsettings.xml (skipped by default)"
      echo "  --skip-system-tweaks  Skip Android system tweaks (animations, DNS, telemetry)"
      echo "  --tivimate-only       Explicit: system tweaks only, no Kodi (this is the default)"
...
XMLEOF\""
    ...
    echo ""
    echo "NOTE: POV autostart is configured through the POV UI, not ADB."
...
if $SKIP_KODI; then
  ...
  echo "  3. Player settings: Tunneled Playback OFF, Audio Passthrough ON, Buffer Small"
  ...
  echo "  3. Enable external scrapers: POV → Settings → Sources → External Scrapers → On"
  echo "  4. Enable POV autostart: POV → Settings → General → Auto Start POV → On"
  echo "  5. Configure Kodi cache: Settings → Services → Caching → 350MB, readfactor 20"
  echo "  6. Optional: Sideload TiviMate for live sports (tivimate.en.uptodown.com/android/download)"
  echo "  7. Optional: Install FLauncher from APKPure (Play Store incompatible on AOSP)"
  echo "  8. Check for Ugoos firmware updates (Settings → OTA Update)"
fi
echo ""
